...
public:
	...
	void DebugString(string chkmsg, string str);
...
public:
	...
	void OnRecvIobDiscreateInput(CTcpSocket * sock, unsigned char * pktbuf, unsigned int pktlen);
	void OnRecvIobDiscreateOutput(CTcpSocket * sock, unsigned char * pktbuf, unsigned int pktlen);
...
public:
	BOOL RunIobCbit();
	void OnIobCbitResult(unsigned char * pktbuf, unsigned int pktlen);
	void SendIobDiscreateInputResult(C16bit high, C16bit low, BOOL ok = TRUE);

protected:
	C16bit m_tIobCbitResult;
	UINT32 m_nIobRs422Result;

public:
	void SendJigLog(string log);
};
