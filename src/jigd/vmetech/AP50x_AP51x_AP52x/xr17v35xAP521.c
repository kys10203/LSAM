...
/* !!!!!!  N O T E :  T O  E N A B L E  R S - 4 2 2 / R S - 4 8 5  O P E R A T I O N !!!!!!!! added FJM */
// Set this parameter to 0x01 to enable RS422/RS485 mode
...
#define XR_17V35X_INT0			0x80 //ECRIN-LTA
#define XR_17V35X_REGB			0x8E //ECRIN-LTA
#define DIS_WAKE_INTR			(1 << 1) //ECRIN-LTA
...
{
//FJM modified struct tty_struct *tty = up->port.state->port.tty;
	struct tty_struct *tty; /* FJM added */
	...
	unsigned char tmp;
    	...
    	int port_index;			/* FJM added */
        tty = up->port.state->port.tty; /* FJM added */
...
//FJM modified int port_index = up->port.line;
...
static int serialxr_startup(struct uart_port *port)
{
	struct uart_xr_port *up = (struct uart_xr_port *)port;
	unsigned long flags;
	unsigned int fctr_reg=0, regb_reg=0;
	int retval;
	...
	//ECRIN-LTA: Disable and clean Wake up interrupts
	regb_reg = serial_in(up,XR_17V35X_REGB);
	serial_out(up, XR_17V35X_REGB, regb_reg | DIS_WAKE_INTR );
	(void) serial_in(up, XR_17V35X_INT0) ;
	//ECRIN-LTA: END
...
if(port->irq) {
	...
	*/
	...
	(void) serial_in(up, UART_IIR);
	(void) serial_in(up, UART_MSR);
	(void) serial_in(up, XR_17V35X_INT0) ; //ECRIN-LTA
...
{
	struct uart_xr_port *up = (struct uart_xr_port *)port;
	signed int baud, quot, quot_fraction = 0;/* initialized FJM */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)	/* added/modified FJM */
                timer_setup(&uart->timer, (void*)serialxr_timeout, 0);
#else	
		init_timer(&uart->timer);
		uart->timer.function = serialxr_timeout;
#endif							/* end added/modified FJM */
		/*
...
static int __init serialxr_init(void)
{
	...
	printk(KERN_INFO "Exar PCIe (XR17V35x) serial driver Revision: 2.2c\n");
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Exar PCIe specific serial driver for XR17V35x- Revision: 2.2c");
